...
0. **作業フロー（恒常運用）**:
   ...
   - ユーザー確認が取れたものだけをコミットし、`main` に push してからデプロイする。
...
- **ステータス**: `To do`
- **カテゴリ**: LOG
- **説明**: AIに読ませる ON/OFF のログをスレッドではなくトークに出し、本文は「[ユーザ]がAIに読ませるをオン・オフにしました」に統一する

#### チャットコンテキスト自動ログの整理
- **ID**: CAT-015
- **ステータス**: `To do`
- **カテゴリ**: LOG
- **説明**: スレッドの `[CHAT_CONTEXT] ユーザ: ...` ログを廃止し、`[CHAT_CONTEXT_STATUS]` 系ログは「何を誰がどうしたか」が分かる日本語文に置き換える

#### AIスレッド上部バーの作成者アイコン表示
- **ID**: CAT-016
- **ステータス**: `To do`
- **カテゴリ**: UI
- **説明**: スレッド上部バーの日付右横に作成者アイコンを潰れない形で表示する
...
- **ステータス**: `To do`
- **カテゴリ**: UI
- **説明**: スレッド上部でタイトル/鉛筆/その他情報を横並びにし、その他情報は右詰め、タイトル/鉛筆は他情報が最大まで圧縮されるまでは圧縮しない

#### AIスレッド背景のグラデーション化
- **ID**: CAT-030
- **ステータス**: `To do`
- **カテゴリ**: UI
- **説明**: AIスレッドの背景を見栄えのよいグラデーションに変更する

#### スレッド名編集時のトップバー簡素化
- **ID**: CAT-031
- **ステータス**: `To do`
- **カテゴリ**: UI
- **説明**: スレッド名編集中はトップバーに入力欄のみを表示する
