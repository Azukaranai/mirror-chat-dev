...
0. **作業フロー（恒常運用）**:
   - 実装は「実装場所（機能/画面/領域）ごと」にローカルへ反映する。
   - 各タスクの実装前に、タスクごとに1文で「これから何をするか」を提示し、ユーザーの「go」合図後に着手する。
   - 複数タスクをまとめてローカルに反映し、ユーザー確認が取れ次第、ユーザー指定のタイミングでコミット→`main` への push→デプロイを行う。
   - ローカル更新後は `http://localhost:3000` にアクセスできるか必ず確認し、確認後に完了報告する。

0. **ローカル接続の維持**:
   - ローカルWebは常に `http://localhost:3000` で実行する。
   - 接続不能が発生した場合は状態確認のうえ、必要ならプロセスを停止して再起動する。
...
- **ステータス**: `Done`
- **完了日**: 2026-01-13
- **説明**: AIに読ませる ON/OFF のログをトーク/スレッドへ整合した日本語文で記録する

#### チャットコンテキスト自動ログの整理
- **ID**: CAT-015
- **ステータス**: `Done`
- **完了日**: 2026-01-13
- **説明**: `[CHAT_CONTEXT]` ログ廃止と、`[CHAT_CONTEXT_STATUS]` を日本語ログへ置換

#### AIスレッド上部バーの作成者アイコン表示
- **ID**: CAT-016
- **ステータス**: `Done`
- **完了日**: 2026-01-13
- **説明**: 日付右横に作成者アイコンを潰れない形で表示

#### スレッドタブのレイアウト最適化
- **ID**: CAT-021
- **ステータス**: `Done`
- **完了日**: 2026-01-13
- **説明**: タイトル/鉛筆/その他情報を1行横並び・右詰めで表示

#### AIスレッド新規作成を共有時に選択可能
- **ID**: FEAT-004
- **ステータス**: `Done`
- **完了日**: 2026-01-13
- **説明**: 友達へのチャットでの共有時に新規作成も選択可能に
